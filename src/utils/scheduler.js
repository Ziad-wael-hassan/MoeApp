  ...
  cron.schedule(
    ...
    async () => {
      try {
        ...
        logger.error(
          { err: error },
          "Error sending scheduled reminder message:",
        );
      }
    },
    {
  ...
  try {
    ...
    for (const reminder of reminders) {
      ...
      if (reminderDate > now) {
        ...
        cron.schedule(
          ...
          async () => {
            try {
              ...
              await Settings.deleteOne({
                key: `reminder_${targetNumber}_${reminderDate.getTime()}`,
              });
        ...
        logger.info(
          `Deleting outdated reminder to ${targetNumber} scheduled for ${reminderDate}`,
        );
        await Settings.deleteOne({
          key: `reminder_${targetNumber}_${reminderDate.getTime()}`,
        });
