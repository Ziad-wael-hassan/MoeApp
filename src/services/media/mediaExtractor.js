        ...
        if (mediaData.status === "picker" && Array.isArray(mediaData.picker)) {
            for (const item of mediaData.picker) {
                if (item.type === "photo" && item.url) {
                    try {
                        // First attempt: Try with base64
                        try {
                            const { base64, mimeType } = await downloadMedia(item.url);
                            const media = new MessageMedia(mimeType, base64);
                            await sendMediaWithRetry(media, message);
                        } catch (base64Error) {
                            logger.warn(`Base64 method failed for picker item, trying MessageMedia.fromUrl: ${base64Error.message}`);
                        ...
                        logger.error(`Failed to process picker item with both methods: ${error.message}`);
                        continue;
                    }
                }
            }
            ...
            for (const mediaUrl of mediaUrls) {
                try {
                    ...
                    try {
                        ...
                        logger.warn(`Base64 method failed, trying MessageMedia.fromUrl: ${base64Error.message}`);
                    ...
                    logger.error(`Failed to process media URL with both methods: ${error.message}`);
                    continue;
                }
            }
        }
